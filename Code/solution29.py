...
class Solution29(Scene):
    def construct(self):
        
        soloution_text = VGroup(
            MathTex(r"\text{The LTI system with impulse response}",color=LIGHT_GRAY),
            MathTex(r"h(t) = \begin{cases} \frac{1}{t + 1} & \text{for } t \geq 0 \\ 0 & \text{otherwise} \end{cases} ", color = BLUE),
        ...
        equation_text = VGroup(
            MathTex(r"\int_0^\infty \frac{1}{t + 1} dt", color = BLUE),
            MathTex(r"= [\log(t + 1)]^{\infty}_{0}", color = BLUE),
            MathTex(r"= \log(\infty + 1) - \log(0 + 1)", color = BLUE),
