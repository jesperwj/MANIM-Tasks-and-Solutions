...
class Solution38(VoiceoverScene):
    def construct(self):
        ...
        question = VGroup( 
                MathTex(r"\text{If an } n \times n \text{ square matrix has } n \text{ different eigenvalues}", 
        ...
        question_and_answers_str = [
            "If an N times N square matrix has N distinct eigenvalues, then it is diagonalizable",
            "Which of the following answers is correct for this statement.",
            "Option A , true. B, false. C, it depends. Or D, I don't know."
        ]
        ...
        with self.voiceover(text=question_and_answers_str[0]) as tracker:
            self.play(Write(question), run_time=tracker.duration)
        with self.voiceover(text=question_and_answers_str[1]) as tracker:
            self.wait(tracker.duration)
        with self.voiceover(text=question_and_answers_str[2]) as tracker:
            self.play(Write(answers), run_time=tracker.duration)
        #self.play(FadeIn(question, answers))
        self.wait(4)

        correct_answer_indicator_str = [
            "The correct answer is A"
        ]

        # Animating the correct answer indication 
        with self.voiceover(text=correct_answer_indicator_str[0]) as tracker:
        ...
        title_tts_str = "To understand why this is the correct answer, let us take a look at Eigenvalues and Eigenvectors"
        ...
        description0_str = [
            "Let's consider a generic matrix A of size N times N, with eigenvalues lambda 1, lambda 2 all the way up to lambda N.",
            "Let lambda I be a generic eigenvalue of A.",
            "V I",
            " is the eigenvector corresponding to",
            "lambda I",
            "so lambda I determines how V I stretches after",
        ...
        # Animations part one
        with self.voiceover(text=title_tts_str) as tracker:
                self.play(Create(title,run_time=tracker.duration))
        
        self.wait(1)
        
        with self.voiceover(text=description0_str[0]) as tracker:
                self.play(Create(description0[0],run_time=tracker.duration))
        #self.wait(1)
        with self.voiceover(text=description0_str[1]) as tracker:
                self.play(Create(description0[1],run_time=tracker.duration))
        #self.wait(1)
        with self.voiceover(text=(description0_str[2] + description0_str[3]  + description0_str[4] ) ) as tracker:
                self.play(Create(description0[2],run_time=tracker.duration))
        #self.wait(1)
        ...
        self.wait(1)
        with self.voiceover(text=description0_str[5]) as tracker:
                self.play(Create(description0[3],run_time=tracker.duration))
        #self.wait(0.5)
        with self.voiceover(text=description0_str[6]) as tracker:
                self.play(Create(description0[4],run_time=tracker.duration))
        #self.wait(1)
        ...
        description1_str = [
            " Now consider a different eigenvalue lambda J",
            " Where lambda J and lambda I are distinct",
            " then let V J be the eigenvector corresponding to lambda J"
        ]

        # Animations part two
        with self.voiceover(text=description1_str[0]) as tracker:
                self.play(Create(description1[3],run_time=tracker.duration))
        #self.play(Create(description1[3],run_time = 1))
        #self.wait(0.5)
        with self.voiceover(text=description1_str[1]) as tracker:
                self.play(Create(description1[4],run_time=tracker.duration))
        #self.play(Create(description1[4],run_time = 1))
        #self.wait(1)
        with self.voiceover(text=description1_str[2]) as tracker:
                self.play(Create(description1[5],run_time=tracker.duration))
        ...
        description2_str = [
            " Eigenvectors of two different eigenvalues",
            " must be linearly independent.",
            " One eigenvector cannot be associated to",
            "two different eigenvalues, because it ",
            "cannot be stretched in two different ways."
        ]

        #Animations
        with self.voiceover(text=description2_str[0]) as tracker:
                self.play(Create(description2[1],run_time=tracker.duration))
        #self.play(Create(description2[1],run_time=1.5))
        #self.wait(0.3)
        with self.voiceover(text=description2_str[1]) as tracker:
                self.play(Create(description2[2],run_time=tracker.duration))
        ...
        with self.voiceover(text=description2_str[2]) as tracker:
                self.play(Create(description2[3],run_time=tracker.duration), Rotate(VGroup(span2, eigenvector2_copy), angle=PI*0.14758, run_time=2, about_point=number_plane_copy2.c2p(0, 0)), FadeOut(span_label2,eigenvector2_copy,span2) )
                #self.play(Create(description2[3],run_time=tracker.duration) )
                #self.play(Rotate(VGroup(span2, eigenvector2_copy), angle=PI*0.14758, run_time=2, about_point=number_plane_copy2.c2p(0, 0)))
                #self.play(FadeOut(span_label2,eigenvector2_copy,span2))
        #self.play(Create(description2[3],run_time=1.5),Rotate(VGroup(span2, eigenvector2_copy), angle=PI*0.14758, run_time=2, about_point=number_plane_copy2.c2p(0, 0)))
        with self.voiceover(text=(description2_str[3] + description2_str[4])) as tracker:
                time_section_1 = len(description2_str[3])/len(description2_str[3] + description2_str[4])
                time_section_2 = len(description2_str[4])/len(description2_str[3] + description2_str[4])
                self.play(Create(description2[4],run_time=time_section_1*tracker.duration))
                self.play(Create(description2[5],run_time=time_section_2*tracker.duration))
        #self.play(Create(description2[5],run_time=1))
        self.play(ReplacementTransform(number_plane_copy2, transformed_plane_copy2),
        ...
        text_break_tts_str = [
                ...
                "If an N times N square matrix has N  different eigenvalues",
                "is it diagonalizable?"
        ]

        # Animation
        self.wait(1)
        with self.voiceover(text=text_break_tts_str[0]) as tracker:
                self.play(Create(text_break[0],run_time=tracker.duration))
        #self.play(Create(text_break[0], run_time = 1))
        #self.wait(1)
        with self.voiceover(text=text_break_tts_str[1]) as tracker:
                self.play(Create(text_break[1],run_time=tracker.duration))
        #self.play(Create(text_break[1], run_time = 1.5))
        #self.wait(0.5)
        with self.voiceover(text=text_break_tts_str[2]) as tracker:
                self.play(Create(text_break[2],run_time=tracker.duration))
