...
class Convolution3(Scene):

    def __init__(self, signal_1, signal_2, signal_1_label=None, signal_2_label=None, title_text=None, limit_x=None, limit_y=None, **kwargs):
        super().__init__(**kwargs)

        self.signal_1 = signal_1
        self.signal_2 = signal_2
        self.signal_1_label = signal_1_label or r"u(t)"
        self.signal_2_label = signal_2_label or r"h(t)"

        self.title_text = title_text if title_text else r"\text{Convolution Visualization}"

        self.limit_x = limit_x if limit_x else [-5, 5]
        self.limit_y = limit_y if limit_y else [-2, 2]
        self.limit_x_conv = self.limit_x
        self.limit_y_conv = self.limit_y
    ...
    def construct(self):
        ...
        # =========================== Signals =================================
        function1 = self.signal_1
        function2 = self.signal_2
    ...
    def create_axes(self):
        ...
        axes1 = Axes([self.limit_x[0], self.limit_x[1],0.25], [self.limit_y[0], self.limit_y[1] + 1, 1], 
                     axis_config=axes_config, x_length=14, y_length=2.93, tips = False).to_edge(UP, buff=0)
        axes2 = Axes([self.limit_x[0], self.limit_x[1],0.25],  [self.limit_y[0]-1, self.limit_y[1]-1, 1], 
                     axis_config=axes_config, x_length=14, y_length=2.4,tips = False).next_to(axes1, DOWN, buff=0)
        axes3 = Axes([self.limit_x_conv[0], self.limit_x_conv[1],0.25], [self.limit_y_conv[0], self.limit_y_conv[1], 1], 
                     axis_config=axes_config, x_length=14, y_length=2.66,tips = False).next_to(axes2, DOWN, buff=0)
        return axes1, axes2, axes3

    def create_labels(self):
        label1 = VGroup(
            MathTex(self.signal_1_label, color=self.colors["function1"], font_size=self.font_size),
            MathTex(self.signal_2_label, color=self.colors["function2"], font_size=self.font_size)
        ).arrange(LEFT, center=False, buff=6.3)
        labels = VGroup(
            label1,
    ...
    def create_title(self):
        ...
        if isinstance(text, str):
            title_tex = MathTex(text, color=WHITE, font_size=self.font_size * 1.2).set_z_index(2)
        else:
            title_tex = text
        
        underline = Underline(title_tex, color=self.colors["axes"]).set_stroke(width=2).set_z_index(2)
        title_box = Rectangle(
            width=title_tex.width + 0.4,  
            height=title_tex.height + 0.27,
            color=BLACK,  
            fill_color=BLACK,  
            fill_opacity=1  
        ).move_to(title_tex.get_center()).set_z_index(1)  
        title = VGroup(title_box, underline, title_tex).to_edge(UP, buff=0)
        return title
